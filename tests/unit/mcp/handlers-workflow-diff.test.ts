...
describe('handlers-workflow-diff', () => {
  ...
  const withMarket = <T extends Record<string, any>>(payload: T) => ({ market: 'CL', ...payload });
  ...
  describe('handleUpdatePartialWorkflow', () => {
    it('should apply diff operations successfully', async () => {
      ...
      const result = await handleUpdatePartialWorkflow(withMarket(diffRequest));
    ...
    it('should handle API not configured error', async () => {
      ...
      const result = await handleUpdatePartialWorkflow(withMarket({
        id: 'test-id',
        operations: [],
      }));
    ...
    it('should handle debug logging when enabled', async () => {
      ...
      await handleUpdatePartialWorkflow(withMarket({
        id: 'test-id',
        operations: [{ type: 'updateNode', nodeId: 'node1', updates: {} }],
      }));
