...
export interface InstanceContext {
  ...
  /**
   * Market identifier (e.g., CL, MX)
   * Used to look up environment-based credentials
   */
  market?: string;
...
export function isInstanceContext(obj: any): obj is InstanceContext {
  ...
  const hasValidMarket = obj.market === undefined || typeof obj.market === 'string';
...
export function validateInstanceContext(context: InstanceContext): {
  ...
  if (context.market !== undefined) {
    if (typeof context.market !== 'string' || context.market.trim().length === 0) {
      errors.push('Invalid market: value must be a non-empty string when provided');
    }
  }
